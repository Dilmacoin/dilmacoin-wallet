...
public final class BitcoinIntegration
{
	...
	{
		final StringBuilder uri = new StringBuilder("dilmacoin:");
		if (address != null)
			uri.append(address);
		if (amount != null)
	...
	{
		Toast.makeText(context, "No Dilmacoin application found.\nPlease install Dilmacoin Wallet.", Toast.LENGTH_LONG).show();

		final Intent marketIntent = new Intent(Intent.ACTION_VIEW, Uri.parse("market://details?id=de.schildbach.wallet"));
		final Intent binaryIntent = new Intent(Intent.ACTION_VIEW, Uri.parse("http://schildbach.github.io/dilmacoin-wallet-new/"));
