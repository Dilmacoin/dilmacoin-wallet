...
public class ExchangeRatesProvider extends ContentProvider
{
    ...
    private float dilmaBtcConversion = -1;
    ...
    private static final URL HUEPOOL_URL;
	...
	static
	{
		try
		{
            ...
            HUEPOOL_URL = new URL("http://pool.dilmacoin.com/lastdilma");
            CRYPTSY_URL = new URL("http://pubapi.cryptsy.com/api.php?method=singlemarketdata&marketid=132");
            VIRCUREX_URL = new URL("https://api.vircurex.com/api/get_last_trade.json?base=HUE&alt=BTC");
		}
	...
	public Cursor query(final Uri uri, final String[] projection, final String selection, final String[] selectionArgs, final String sortOrder)
	{
		...
		{
            float newHueBtcConversion = -1;
            if ((dilmaBtcConversion == -1 && newHueBtcConversion == -1) || forceRefresh)
                newHueBtcConversion = requestHueBtcConversion(provider);

            if (newHueBtcConversion != -1)
                dilmaBtcConversion = newHueBtcConversion;

            if (dilmaBtcConversion == -1)
                return null;

			Map<String, ExchangeRate> newExchangeRates = null;
			if (newExchangeRates == null)
				newExchangeRates = requestExchangeRates(BITCOINAVERAGE_URL, dilmaBtcConversion, userAgent, BITCOINAVERAGE_SOURCE, BITCOINAVERAGE_FIELDS);
			if (newExchangeRates == null)
				newExchangeRates = requestExchangeRates(BLOCKCHAININFO_URL, dilmaBtcConversion, userAgent, BLOCKCHAININFO_SOURCE, BLOCKCHAININFO_FIELDS);
			...
			if (newExchangeRates != null)
			{
                ...
                float mBTCRate = dilmaBtcConversion*1000;
                ...
                newExchangeRates.put("HUE", new ExchangeRate("HUE", BigInteger.valueOf(100000000), "priceofdilma.com"));
		...
		if (exchangeRates == null || dilmaBtcConversion == -1)
			return null;
	...
	private static Map<String, ExchangeRate> requestExchangeRates(final URL url, float dilmaBtcConversion, final String userAgent, final String source, final String... fields)
	{
						...
						for (final String field : fields)
						{
							...
							if (rate != null)
							{
								try
								{
                                	...
                                	BigInteger dilmaRate = btcRate.multiply(BigDecimal.valueOf(dilmaBtcConversion)).toBigInteger();

									if (dilmaRate.signum() > 0)
									{
										rates.put(currencyCode, new ExchangeRate(currencyCode, dilmaRate, source));
										break;
									}
								}
    ...
    private static float requestHueBtcConversion(int provider) {
        HttpURLConnection connection = null;
        Reader reader = null;
        URL providerUrl;
        switch (provider) {
            case 0:
                providerUrl = HUEPOOL_URL;
                break;
            case 1:
